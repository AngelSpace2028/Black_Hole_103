...
# === XOR with Prime (every 3 bytes, repeated 50 times) ===
def transform_with_prime_xor_every_3_bytes(data, repeat=50):
    transformed = bytearray(data)
    total_ops = len(PRIMES) * repeat
    pbar = tqdm(total=total_ops, desc="XOR with Primes", unit="ops")
    for prime in PRIMES:
...
class SmartCompressor:
    ...
    def reverse_reversible_transform(self, data):
        return transform_with_prime_xor_every_3_bytes(data)  # XOR is symmetric
...
# === Simple Encoder/Decoder with PAQ ===
